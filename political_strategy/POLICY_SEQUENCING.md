# POLICY SEQUENCING: The Order of Operations
## How Human Standard Policies Interact and Must Be Ordered
### By Zeno

---

## EXECUTIVE SUMMARY

Policies don't exist in isolation. The order in which Human Standard policies are implemented determines whether they succeed or fail. This document maps the dependencies between policies and provides the optimal sequencing strategy.

**The Core Insight:** Some policies are prerequisites for others. Some policies can undermine each other if sequenced incorrectly. Getting the order right is as important as getting the policies right.

---

## PART I: DEPENDENCY MAPPING

### 1.1 Policy Dependency Matrix

```
LEGEND:
→ = Must come before
⟷ = Should be simultaneous
✕ = Conflicts if wrong order
```

| Policy | Prerequisites | Conflicts Without | Enables |
|--------|--------------|-------------------|---------|
| Citizen's Royalty | Tax infrastructure, Payment systems | Housing (inflation), Healthcare | Consumer demand, Risk-taking |
| Automation Tax | Disclosure framework, International coordination | Capital flight if no coordination | Citizen's Royalty funding |
| AI Governance | NAICO establishment, Standards development | Open source if security not ready | Trust, Adoption |
| Housing Reform | Political capital, State adoption | Citizen's Royalty (inflation without supply) | Citizen's Royalty effectiveness |
| Healthcare | Political capital, Funding | Citizen's Royalty (incomplete without it) | Worker mobility |
| Sound Money | Reserve establishment, Infrastructure | Citizen's Royalty if volatility not addressed | Long-term stability |
| Data Tax | Platform regulation, Measurement systems | Revenue if evasion not addressed | Citizen's Royalty funding |
| Carbon Tax | Economic modeling, Border adjustments | Competitiveness if unilateral | Climate, Citizen's Royalty funding |

### 1.2 Critical Dependencies

#### Citizen's Royalty Requires Housing Reform First (Or Simultaneously)

**The Problem:**
- Citizen's Royalty puts $24,000/year into every adult's pocket
- Without housing supply increase, landlords capture much of this
- Rents rise to absorb new income
- Real benefit to recipients diminishes

**Evidence:**
- Studies of cash transfer programs show rent increases
- Section 8 voucher research shows landlord capture
- Alaska Permanent Fund has seen housing inflation correlation

**The Solution:**
- Pass housing/zoning reform BEFORE or WITH Citizen's Royalty expansion
- Phase Citizen's Royalty in gradually to allow supply response
- Target housing supply increase in Citizen's Royalty ramp-up areas
- Include rent stabilization transition measures

**Sequencing:**
1. Housing reform legislation (Year 1)
2. Zoning changes take effect (Year 2)
3. Construction ramp-up (Years 2-4)
4. Citizen's Royalty expansion aligned with housing supply (Years 3-5)

#### Automation Tax Requires International Coordination

**The Problem:**
- Unilateral automation tax creates competitive disadvantage
- Companies can offshore automation
- Revenue projections become unreliable
- Political argument against policy strengthens

**Evidence:**
- Corporate inversions showed capital mobility
- State tax competition demonstrates race to bottom
- Carbon tax debate shows competitiveness concerns

**The Solution:**
- Begin international coordination before full tax implementation
- Use border adjustments for non-cooperating countries
- Build coalition of willing countries first
- Phase in domestically while building international framework

**Sequencing:**
1. Disclosure requirements (Year 1) - data gathering, no tax
2. International negotiations (Years 1-3)
3. Initial low tax rate (Year 3) - with border adjustments
4. Coalition adoption (Years 3-5)
5. Full tax rate (Year 5+) - with broad coalition

#### AI Governance Requires Security Framework First

**The Problem:**
- Transparency requirements without security hardening creates vulnerabilities
- Adversaries could exploit published code
- Trust in government AI undermined by breaches
- Political backlash against transparency

**The Solution:**
- Establish security standards before transparency requirements
- Phase transparency by system criticality
- Build security infrastructure in parallel
- Demonstrate secure open source success stories

**Sequencing:**
1. Security framework establishment (Year 1)
2. Tier 1 systems open sourced (Year 2)
3. Security hardening for Tier 2 (Years 2-3)
4. Delayed disclosure for Tier 2 begins (Year 3)
5. Full framework operational (Year 4)

---

## PART II: THE OPTIMAL SEQUENCE

### 2.1 Phase 0: Preparation (Before Taking Office)

**Policy Development:**
- Final policy documents completed
- Model legislation drafted
- Constitutional analysis completed
- Litigation strategy prepared

**Coalition Building:**
- Key legislators committed
- Industry allies identified
- Advocacy organizations aligned
- Media strategy prepared

**Personnel:**
- Cabinet nominees vetted
- Agency leadership identified
- Transition team trained
- Day One orders drafted

### 2.2 Phase 1: Foundation (Months 1-12)

**Day One Executive Actions:**
1. Establish NAICO (Executive Order)
2. Federal AI transparency requirements (Executive Order)
3. Automation impact assessment requirement (Executive Order)
4. Diversified Strategic Reserve directive (Treasury)
5. Data dignity principles (Agency guidance)

**Month 1-3 Regulatory:**
- Begin rulemaking for AI governance
- Issue FAR amendments for contractor transparency
- Launch data protection review
- Initiate automation tax framework study

**Month 1-6 Legislative:**
- Introduce Foundation Package (Package A)
- Automation tax disclosure requirement
- Data tax framework
- Citizen's Royalty pilot authorization
- NAICO statutory authority

**Month 6-12 Implementation:**
- Pass Foundation Package (reconciliation path)
- Launch Citizen's Royalty pilot program
- NAICO fully operational
- Automation Observatory collecting data
- First transparency rules effective

### 2.3 Phase 2: Critical Prerequisites (Months 12-24)

**Housing Reform Push:**
- Federal incentives for zoning reform
- Model state legislation distribution
- YIMBY coalition mobilization
- First state adoptions

**Healthcare Foundation:**
- Public option introduction
- Medicare negotiation (inflation control)
- Healthcare workforce expansion
- Mental health parity enforcement

**International Coordination:**
- G7 automation tax discussions
- OECD digital tax alignment
- EU AI governance cooperation
- Trade agreement updates

**Security Infrastructure:**
- AI security standards finalized
- First government systems hardened
- Bug bounty program launched
- Independent audits initiated

### 2.4 Phase 3: Expansion (Months 24-48)

**Year 3 Priorities:**
- Automation tax at 5% (reconciliation)
- Carbon tax at $50/ton
- Citizen's Royalty Phase 2 expansion
- AI governance comprehensive bill
- First open source releases

**Year 4 Priorities:**
- Housing reform in 10+ states
- International coalition at 5+ countries
- Full AI governance regulations
- Citizen's Royalty reaching 50 million recipients
- Wealth tax introduction (if viable)

**Critical Checks:**
- Housing supply response adequate?
- Inflation controlled?
- Revenue meeting projections?
- AI governance working?
- International coordination advancing?

### 2.5 Phase 4: Full Implementation (Months 48-84)

**Year 5-6 Priorities:**
- Automation tax to 15%
- Citizen's Royalty to $1,500/month
- Program consolidation begins
- Constitutional amendment introduced
- Full AI governance operational

**Year 7 Priorities:**
- Citizen's Royalty at $24,000/year
- Full program consolidation
- Stabilization fund established
- Amendment ratification campaign
- International framework complete

---

## PART III: INTERACTION ANALYSIS

### 3.1 Positive Interactions (Reinforcing)

**Citizen's Royalty + Automation Tax:**
- Automation tax funds Citizen's Royalty
- Citizen's Royalty maintains consumer demand
- Consumer demand supports businesses paying automation tax
- Virtuous cycle

**AI Governance + Sound Money:**
- Transparent governance builds trust
- Trust enables digital currency adoption
- Digital infrastructure enables transparent governance
- Mutual reinforcement

**Housing Reform + Citizen's Royalty:**
- Housing reform increases supply
- Increased supply controls prices
- Citizen's Royalty benefit preserved for recipients
- Economic mobility increases

**Data Tax + AI Governance:**
- Data tax creates revenue
- Revenue funds governance infrastructure
- Governance legitimizes data economy
- Virtuous cycle

### 3.2 Negative Interactions (Requiring Management)

**Citizen's Royalty + Healthcare Costs:**
- **Risk:** Citizen's Royalty increases demand for healthcare, prices rise
- **Management:** Healthcare cost controls in parallel
- **Sequencing:** Healthcare reform with or before Citizen's Royalty expansion

**Automation Tax + Competitiveness:**
- **Risk:** Tax makes U.S. less competitive
- **Management:** International coordination, border adjustments
- **Sequencing:** Coordination before full tax rates

**Sound Money + Tax Collection:**
- **Risk:** Crypto privacy undermines tax compliance
- **Management:** Fiscal architecture resolution (see FISCAL_ARCHITECTURE.md)
- **Sequencing:** Compliance infrastructure with adoption

**Open Source + Security:**
- **Risk:** Transparency creates vulnerabilities
- **Management:** Tiered security framework (see AI_SECURITY_FRAMEWORK.md)
- **Sequencing:** Security standards before open sourcing

### 3.3 Unintended Consequences to Monitor

| Policy | Potential Unintended Consequence | Early Warning Signs | Mitigation |
|--------|----------------------------------|---------------------|------------|
| Citizen's Royalty | Work disincentive | Labor force participation drops | Adjust amount, add incentives |
| Citizen's Royalty | Inflation | CPI acceleration | Monetary policy, supply expansion |
| Automation Tax | Capital flight | Investment metrics drop | Adjust rates, enforcement |
| Automation Tax | Innovation slowdown | R&D spending drops | Targeted credits, exemptions |
| AI Governance | Regulatory burden | Compliance costs rise | Streamline requirements |
| AI Governance | Talent flight | Tech workers leave | Balance with innovation support |
| Data Tax | Service degradation | Free services reduced | Consumer protection |
| Carbon Tax | Energy poverty | Low-income energy burden | Dividend distribution priority |
| Housing Reform | Construction labor shortage | Wages spike, delays | Workforce development |
| Sound Money | Volatility | Citizen's Royalty purchasing power swings | Stablecoin focus, reserves |

---

## PART IV: SCENARIO-BASED SEQUENCING

### 4.1 If Automation Is Faster Than Expected

**Adjusted Sequence:**
- Accelerate Citizen's Royalty pilot to full program
- Emergency automation tax implementation
- Fast-track AI governance
- Delay housing reform if necessary (emergency override)
- Emergency job guarantee as bridge

**Trigger Points:**
- Unemployment rises above 7%
- Robot deployment exceeds 500,000 units
- AI displaces 5+ million jobs in 12 months

### 4.2 If Automation Is Slower Than Expected

**Adjusted Sequence:**
- Delay full automation tax (stay at disclosure/low rate)
- Focus on alternative revenue sources
- Expand pilot longer before full Citizen's Royalty
- Use time to complete housing reform
- Build international coalition thoroughly

**Trigger Points:**
- Robot deployment under 100,000 by 2030
- Displacement under projections
- Revenue from automation tax underperforming

### 4.3 If Political Resistance Is Stronger Than Expected

**Adjusted Sequence:**
- Focus on state-level implementation
- Maximize executive action
- Build coalition for later legislation
- Accept smaller initial steps
- Maintain long-term vision

**Trigger Points:**
- Foundation Package fails in Congress
- Midterm elections unfavorable
- Key allies lose seats

### 4.4 If Revenue Falls Short

**Adjusted Sequence:**
- Scale Citizen's Royalty to available funding
- Accelerate alternative revenue sources
- Prioritize consumption tax and carbon tax
- Delay wealth tax if unconstitutional
- Maintain minimum guarantee

**Trigger Points:**
- Revenue 20%+ below projections
- Constitutional challenge succeeds
- Capital flight exceeds 10% of tax base

---

## PART V: DECISION TREES

### 5.1 Year 2 Decision Tree

```
Did Foundation Package pass?
├── YES
│   ├── Is Citizen's Royalty pilot successful?
│   │   ├── YES → Proceed to Expansion Package
│   │   └── NO → Analyze and adjust before expanding
│   └── Is automation tax disclosure working?
│       ├── YES → Prepare for tax phase-in
│       └── NO → Improve compliance before tax phase-in
└── NO
    ├── Is it filibustered?
    │   ├── YES → Use reconciliation for budget items
    │   └── NO → Address substantive objections
    └── Is executive action sufficient?
        ├── YES → Proceed with executive strategy
        └── NO → Focus on state-level and coalition building
```

### 5.2 Year 4 Decision Tree

```
Is housing supply responding?
├── YES → Proceed with Citizen's Royalty expansion
└── NO
    ├── Is inflation controlled?
    │   ├── YES → Proceed cautiously with Citizen's Royalty expansion
    │   └── NO → Pause Citizen's Royalty expansion, accelerate housing
    └── Are rents stable?
        ├── YES → Citizen's Royalty benefit preserved, continue
        └── NO → Implement rent stabilization, boost supply

Is international coalition forming?
├── YES → Proceed to full automation tax
└── NO
    ├── Are border adjustments working?
    │   ├── YES → Proceed unilaterally with protection
    │   └── NO → Slow automation tax, focus on coordination
    └── Is competitiveness threatened?
        ├── YES → Adjust tax rates, enhance incentives
        └── NO → Continue current course
```

### 5.3 Year 7 Decision Tree

```
Is Citizen's Royalty at full implementation?
├── YES
│   ├── Are outcomes meeting targets?
│   │   ├── YES → Begin constitutional amendment process
│   │   └── NO → Analyze and adjust before cementing
│   └── Is public support strong?
│       ├── YES → Constitutional amendment viable
│       └── NO → Focus on demonstrating benefits
└── NO
    ├── Is it funding constrained?
    │   ├── YES → Accelerate alternative revenue
    │   └── NO → Address implementation barriers
    └── Is it politically blocked?
        ├── YES → State-level expansion, electoral strategy
        └── NO → Continue implementation push
```

---

## PART VI: KEY MILESTONES AND METRICS

### 6.1 Phase 1 Success Metrics

| Milestone | Target | Measurement |
|-----------|--------|-------------|
| NAICO operational | Month 6 | Staff hired, budget allocated |
| Automation disclosure | 80% compliance | Business reporting rates |
| Citizen's Royalty pilot launch | 1M recipients | Enrollment numbers |
| AI transparency | 50% of federal systems | Audit results |
| State adoptions | 3 states | Legislation passed |

### 6.2 Phase 2 Success Metrics

| Milestone | Target | Measurement |
|-----------|--------|-------------|
| Housing reform | 10 states | Zoning changes enacted |
| International coalition | 5 countries | Formal agreements |
| Revenue on target | Within 10% | Tax collections |
| Inflation controlled | Under 4% | CPI |
| Automation tax | 5% collected | Revenue |

### 6.3 Phase 3/4 Success Metrics

| Milestone | Target | Measurement |
|-----------|--------|-------------|
| Citizen's Royalty at full rate | $24,000/year | Payment amounts |
| Housing supply | +20% in target areas | Construction permits |
| Poverty reduction | 50% decrease | Census data |
| AI governance | 100% federal compliance | Audit results |
| International | 20+ countries | Coalition membership |
| Amendment progress | 34 states committed | State resolutions |

---

## CONCLUSION

**Sequencing Matters:**

The Human Standard's success depends on getting the order right:

1. **Foundation first:** Build infrastructure before expansion
2. **Prerequisites before dependents:** Housing before Citizen's Royalty, security before transparency
3. **International coordination before full taxation:** Prevent capital flight
4. **Monitoring throughout:** Adjust based on real-world feedback
5. **Contingency plans ready:** Adapt to unexpected developments

**The Optimal Path:**

**Phase 0:** Preparation (pre-office)
**Phase 1:** Foundation (Year 1) - NAICO, disclosure, pilots
**Phase 2:** Prerequisites (Year 2) - Housing, healthcare, international
**Phase 3:** Expansion (Years 3-4) - Taxes, Citizen's Royalty growth, governance
**Phase 4:** Full Implementation (Years 5-7) - Complete system
**Phase 5:** Consolidation (Years 8+) - Constitutional protection

**Flexibility Is Key:**

This sequence is a plan, not a prison. Real-world feedback will require adjustments. The key is understanding the dependencies so we know what we CAN'T skip and what we can adjust.

---

*"Sequence is strategy. Order is outcome."*

—Zeno
